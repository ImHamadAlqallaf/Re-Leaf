        ...
        if let path = Bundle.main.path(forResource: "localData", ofType: "json"),
           let jsonData = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            do {
                ...
                recommendedItems = decodedData.stores.first?.products ?? []
                forYouItems = decodedData.stores.last?.products ?? []
                previousPurchasesItems = decodedData.stores.flatMap { $0.products }
